...
$CharStats[0][1] = $Top                      ;Current Health Pixel y coord
$CharStats[1][1] = $Top                      ;Current Mana Pixel y coord
...
Func UpdateStats()
	...
	ElseIf $CharStats[0][1] <> $Top Then
		If PixelGetColor($HealthX, $CharStats[0][1] - 1) = $BarColor[0] Then GetStats("Life")
	EndIf
	...
	ElseIf $CharStats[1][1] <> $Top Then
		If PixelGetColor($ManaX, $CharStats[1][1] - 1) = $BarColor[1] Then GetStats("Mana")
	EndIf
	...
	If $UseMerc = "Yes" Then
		...
		ElseIf $CharStats[2][1] <> $MercBarRight Then
			If PixelGetColor($CharStats[2][1] + 1, $MercBarY) <> 0 Then GetStats("Merc")
		EndIf
	EndIf
EndFunc   ;==>UpdateStats
...
Func GetStats($CheckType)
	...
	If $DrinkTimer = 1 Then
		If TimerDiff($DrinkTimeStamp) > $DrinkDelay Then $DrinkTimer = 0
	EndIf
...
Func Drink($DrinkType) ;Type of drink. 0 = Full rejuv, 1 = Regular rejuv, 2 = Merc
	...
	$LogFile = FileOpen($BotDir & "\Logs\Events.txt", 1)
	For $N = 0 To 3
		Select
			Case $BeltData[$N] = 1
				FileWrite($LogFile, "(F)")
